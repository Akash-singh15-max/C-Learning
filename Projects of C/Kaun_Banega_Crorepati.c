...
{
    srand(time(NULL));
    printf("\n\t\t%sChalo Khelte hain KAUN BANEGA CROREPATI !!!%s",PINK,COLOR_END);
    Question* questions;
...
int read_questions(char* file_name,Question** questions){
    ...
    *questions= (Question *)malloc(no_of_questions*sizeof(Question));
    ...
    for (int i = 0; i < no_of_questions; i++)
    {
        ...
        (*questions)[i].timeout=atoi(timeout);// atoi is used to make number from string
